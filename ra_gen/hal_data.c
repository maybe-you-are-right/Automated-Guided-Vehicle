...
{ .mode = TIMER_MODE_PWM,
/* Actual period: 0.0000125 seconds. Actual duty: 0%. */.period_counts = (uint32_t) 0x4e2,
...
{ .mode = TIMER_MODE_PWM,
/* Actual period: 0.0000125 seconds. Actual duty: 0%. */.period_counts = (uint32_t) 0x4e2,
  .duty_cycle_counts = 0x0, .source_div = (timer_source_div_t) 0, .channel = 2, .p_callback = NULL,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
